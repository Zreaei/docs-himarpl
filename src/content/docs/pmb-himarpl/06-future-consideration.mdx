...
- Try to force Advokastra Staffs to make an article news in the [blog.himarpl.org](https://blog.himarpl.org) and tag the article news with `berita pmb`, `snbt`, `snbp`, `pmb`, `prestasi istimewa`, `sm upi`. This will help the website to get more traffic from search engines, especially for `pmb himarpl` related keywords.
